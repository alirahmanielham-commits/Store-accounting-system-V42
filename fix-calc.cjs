const fs = require('fs');
const content = fs.readFileSync('src/components/payroll/PayslipsManager.tsx', 'utf8');

const correctCode = `export const calculatePayslipDetails = (att: any, order: any, person: any) => {
...
const regex = /export const calculatePayslipDetails = \([\s\S]*?return \{ pItems[\s\S]*?\};\n};/;
const newContent = content.replace(regex, correctCode);
fs.writeFileSync('src/components/payroll/PayslipsManager.tsx', newContent);
console.log('Fixed');
