...
# We need to extract the modals HTML and put it in a separate variable or render it before returning viewingOrder, 
# or just duplicate it, or better: wrap everything in a single return, but it's currently using early returns.
# Let's just place the modals inside the viewingOrder return block as well.

modals_html = """      {/* Delete Confirmation Modal */}
      {deletingOrder && (
...
# Find the end of the viewingOrder return block.
# It ends with:
#                 )}
#               </div>
#             </div>
...
replacement = target + modals_html + "\n      </div>\n    );\n  }"

# Replace the first occurrence of the end of the viewingOrder block
content = content.replace(target + "      </div>\n    );\n  }", replacement)

with open('src/components/payroll/EmployeeOrdersManager.tsx', 'w', encoding='utf-8') as f:
    f.write(content)

print("Modals added to viewingOrder block too.")
