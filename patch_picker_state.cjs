...
const targetStr = `  const [isRestoreModalOpen, setIsRestoreModalOpen] = useState(false);
...
const replacementStr = `  const [isRestoreModalOpen, setIsRestoreModalOpen] = useState(false);
  const [selectedBackupForRestore, setSelectedBackupForRestore] = useState<any>(null);
  const [restoreState, setRestoreState] = useState<'confirm' | 'progress' | 'success' | 'error'>('confirm');
  const [restoreProgress, setRestoreProgress] = useState(0);
