...
export const addInvoice = async (invoice: any, skipRecalc: boolean = false) => {
  ...
  if (newInvoice.invoiceNumber && !invoice.isAutoGenerated) {
  ...
  if (newInvoice.isDraft || newInvoice.status === 'draft' || newInvoice.isAutoGenerated || newInvoice.type.startsWith('warehouse_')) {
    return newInvoice;
  }
